...
function give_google_analytics_donation_form() {
	...
	<script type="text/javascript">
			...
			/**
			 * GiveWP - GA Enhance Ecommerce tracking.
			 */
			jQuery.noConflict();
			(function( $ ) {
...
/**
 * GA Refund tracking check for UA ID.
 *
 * Inserts a note if refund tracking is turned on but missing a UA-ID.
 *
 * @param $do_change
 * @param $payment_id
 * @param $new_status
 ...
 * @return mixed
 */
function give_google_analytics_refund_tracking( $do_change, $payment_id, $new_status, $old_status ) {
	...
	if ( empty( $ua_code ) ) {
		give_insert_payment_note( $payment_id, __( 'Google Analytics refund tracking beacon could not send because the UA code is missing in Give\'s settings', 'give-google-analytics' ) );

		return $do_change;
	}

	// Important to always return $do_change!!
	return $do_change;
 ...
 * @return bool
 */
function give_google_analytics_send_refund_beacon( $payment_id ) {
	...
	give_insert_payment_note( $payment_id, __( 'Google Analytics donation refund tracking beacon sent.', 'give-google-analytics' ) );
...
function give_google_analytics_completed_donation( $payment_id, $new_status, $old_status ) {
	...
	if ( 
		'pending' === $old_status 
	     && 'publish' === $new_status ) {
		give_google_analytics_record_offsite_payment( $payment_id );
	}
 ...
 * @return string
 */
function give_google_analytics_record_offsite_payment( $payment_id ) {
